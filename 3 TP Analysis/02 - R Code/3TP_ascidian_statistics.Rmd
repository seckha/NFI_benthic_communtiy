...
likelihood_ratio_statistic <- function(data, indices) {
  data_subset <- data[indices, ]
  m_null <- glmmTMB(percent_cover ~ TP + Site + (1|Transect_ID), ziformula = ~1, data = ascd.transect.percent.cover, family=beta_family())
  
  # Compute the likelihood ratio statistic
  LRT_statistic <- 2 * (logLik(m_null) - logLik(m.hurd))
  
  return(LRT_statistic)
}

set.seed(123)  # for reproducibility
bootstrap_results <- boot(data = ascd.transect.percent.cover, 
                          statistic = likelihood_ratio_statistic, 
                          R = 1000)

summary(bootstrap_results)

# Get the original likelihood ratio statistic
t0 <- bootstrap_results$t0

# Calculate a p-value for the interaction effect of TP and Site
p_value_interaction <- mean(bootstrap_results$t >= t0)
